...
license: MIT
compatibility: { Client compatibility (e.g. Claude Code) }
metadata:
  version: "1.0"
  triggers:
    - "{ trigger phrase 1 }"
...
# If using documentation-only skill:
# Instructions are processed directly by the agent from SKILL.md and its references.
```
````
